...
async def h_vm_shell(a):
    ...
    try:
        out, _ = await asyncio.wait_for(p.communicate(), timeout=1500)
    except asyncio.TimeoutError:
        p.kill()
        return "guest command timed out after 1500s"
    return out.decode(errors="replace").strip()[:80000] or f"exit {p.returncode}"
